# Proofr — Chrome Web Store listing

Everything below is copy-paste ready. The store weights the **title** and the **first paragraph** most, so both front-load the words people actually search.
...
```
Optional keyword-heavier variant (still under 75):
```
Proofr – Private AI Proofreader, Rewriter & Translator
```
...
## Category
`Productivity` (secondary intent: Writing / Communication)
...
2. The **tone grid** (your differentiator: one click, many tones).
3. **Translate** in action.
4. A callout: **"100% on‑device · no account · no API key."**
5. It working inside **Gmail / Google Docs** (proves "works where you write").
> Do NOT lead with the API‑key setup screen (that's what sank v1).

## Relaunch checklist (ranking is driven by active users + ratings, not just installs)
- [ ] Ship the on‑device‑first version so first‑run needs zero setup.
- [ ] Ask 5–10 people to install AND use it a few times (weekly active users rank higher than raw installs).
- [ ] Get the first ~5 honest reviews — social proof unlocks ranking.
- [ ] Fill all screenshot slots + a short promo tile.
- [ ] Add a simple privacy policy page stating "on‑device, no data collected" (also required for the store).
- [ ] Post it where the audience is: r/chrome_extensions, r/productivity, indie/PH dev communities, Product Hunt.
